...
export default class CharacterCreateScene extends Phaser.Scene {
    ...
    create() {
        ...
        // Error message label
        this.nameErrorText = this.add.text(leftColW / 2, nameH - 12, '', { 
            fontSize: '10px', color: '#ff4d4d', fontStyle: 'bold' 
        }).setOrigin(0.5);
        ...
        statKeys.forEach((key, i) => {
            ...
            const valText = this.add.text(controlCenterWorldX, y + 8, '1', { fontSize: '18px', color: '#fff', fontStyle: 'bold' }).setOrigin(0.5);
        ...
        this.confirmBtn = makeRoundButton(this, width - 60, height - 60, 30, 0x12a150, '✓', () => {
            ...
            if (!nameStatus.valid) {
                const randomName = Phaser.Utils.Array.GetRandom(this.DUMMY_NAMES);
                this.nameValue = randomName;
                if (this.nameInput) (this.nameInput.node as HTMLInputElement).value = randomName;
                if (this.nameErrorText) {
                    this.nameErrorText.setText("AUTO-CORRECTED: " + nameStatus.error);
                    this.time.delayedCall(2000, () => this.nameErrorText?.setText(''));
                }
            } else {
